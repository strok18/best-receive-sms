...
class ApiController extends Controller
{
    ...
    {
        $this->messageKey = Config::get('cache.prefix') . 'message:';
    }
    ...
    protected function disposeCurlData($phone_num, $warehouse, $data)
    {
        ...
        if (!is_array($data)) {
            ...
            $cache_msg = $redis_sync->checkZset($this->messageKey . $phone_num);
    ...
    public function msgRedis($phone_num, $data)
    {
        ...
        if ($number > 20) {
            //加入待处理列表
            $redis->setSetValue($this->messageKey . 'queue', $phone_num);
        }
    ...
    protected function getMsgCache($phone_num, $num = 19){
        $redis = new RedisController('sync');
        $result = $redis->zRevRange($this->messageKey . $phone_num, 0, $num);
        $data = array();
